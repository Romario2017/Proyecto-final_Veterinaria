...
public class CitaController {
	...
	@RequestMapping("/listaCitaPorEspecialista")
	@ResponseBody
	public List<Cita> listaCitaPorEspecialista(Integer codigo_usuario) {
		return service.listaCitaPorEspecialista(codigo_usuario);
	}

	
	@RequestMapping("/obtenercita")
	@ResponseBody
	public List<Cita> obtenercita(Integer codigoCita) {
		return service.obtenercita(codigoCita);
	}
	

	@RequestMapping("/obtenerhistorialclinico")
	@ResponseBody
	public List<DetalleCitaHistorialClinico> obtenerhistorialclinico(Integer codigoCita) {
		return service.obtenerhistorialclinico(codigoCita);
	}
	
	
	
	@RequestMapping("/insertarHistorialClinico")
	@ResponseBody
	public HistorialClinicoDto insertarHistorialClinico(HistorialClinicoDto historiTemp) {
		
		
		service.insertarHistorialClinico(historiTemp);
		HistorialClinicoDto response=new HistorialClinicoDto();
		return response;
	}
	...
	public Map<String, Object> registra(Cita objCita) {
		...
		System.out.println("esta es la fechaa actual"+new Date());
		java.util.Date date = new Date();
		Object param = new java.sql.Timestamp(date.getTime());
		System.out.println("time stap"+new java.sql.Timestamp(date.getTime())); 
		
		 
		objCita.setFecha_solicitud_cita(new Date());
		//new java.sql.Timestamp(date.getTime())
		
		System.out.println("fech seteada"+objCita.getFecha_solicitud_cita());
	...
	@RequestMapping("/obtenerProximaCita")
	@ResponseBody
	public Date obtenerProximaCita(Integer codigo_usuario) {
		Date  proximacita = new java.util.Date();
		return service.obtenerProximaCita(codigo_usuario) ;//service.listaCitaPorUsuario(codigo_usuario);
	}
	
	@RequestMapping("/obtenerUltimoPedido")
	@ResponseBody
	public Date obtenerUltimoPedido(Integer codigo_usuario) {
		Date  ultimopedido = new java.util.Date();
		return service.obtenerUltimoPedido(codigo_usuario);
	}
}
